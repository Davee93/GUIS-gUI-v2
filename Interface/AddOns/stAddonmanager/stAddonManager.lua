...
-- MEDIA & CONFIG ------------------------------------
------------------------------------------------------
local font = { GameTooltipText:GetFont(), 13, "OUTLINE" }
...
local backdropcolor = {0.05, 0.05, 0.05, 0.6}
local backdrop = {
	bgFile = blankTex, 
	edgeFile =  blankTex, 
...
local function CreateMenuButton(parent, width, height, text, ...)
	...
	button:SetScript("OnEnter", function(self)
		if text == "x" then
			...
			self.text:SetTextColor(unpack(classcolor[playerclass]))
		end
...
function stAddonManager:LoadProfileWindow()
	...
	EnableAll:SetScript("OnClick", function(self)
		for i, addon in pairs(stAddonManager.AllAddons) do
			...
			stAddonManager.Buttons[i]:SetBackdropColor(unpack(classcolor[playerclass]))
			addon.enabled = true
		end
	end)
...
function stAddonManager:LoadWindow()
	...
	for i, addon in pairs(stAddonManager.AllAddons) do
		...
		if addon.enabled then
			button:SetBackdropColor(unpack(classcolor[playerclass]))
		end
		...
		button:SetScript("OnMouseDown", function(self)
			if addon.enabled then
				...
				self:SetBackdropColor(unpack(classcolor[playerclass]))
				EnableAddOn(addon.name)
				addon.enabled = true
			end
		end)
	...
	sbClear:SetScript("OnEnter", function(self) self.text:SetTextColor(1,0,0) end)
