...
function stAM.Initialize(self, event, ...)
	...
	addons:RemoveTextures()
	addons:SetUITemplate("button")
...
function stAM.LoadWindow(self)
	...
	--Close button
	self.close = st.CreateButton(title:GetName()..'_CloseButton', title, 16, 16, {'TOPRIGHT', 7, 7}, '', function() stAM:Hide() end)
	--self.close:RemoveTextures()
	self.close:SetUITemplate("closebutton")
	...
	--profilesmenu:SetUITemplate("backdrop")

	--Search Bar
	local search = st.CreateEditBox(self:GetName()..'_SearchBar', self, 1, 20)
	--search:RemoveTextures()
	--search:SetUITemplate("backdrop")
